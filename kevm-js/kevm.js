...
nconf.use('file', { file: './myData.json' });
nconf.load();
if (nconf.get('privateKey') != undefined ) {
	PRIVATEKEY = nconf.get('privateKey');
    console.log('Existing personal wallet privateKey found in myData.json');
} else {
    console.log('No personal wallet privateKey found in myData.json');
}
if (nconf.get('contractAddress') != undefined ) {
	CONTRACTADDRESS = nconf.get('contractAddress');
    console.log('Existing deployed contractAddress found in myData.json');
} else {
    console.log('No deployed contractAddress found in myData.json');
}
...
const run = async () => {
  ...
  try {
	...
	nconf.save(function (err) {
		if (err) {
			...
			console.log("A new random PRIVATEKEY was generated and stored in ./myData.json");
			console.log("This key will be used now and for future operations");
		}
	  });
  } 
  ...
  if (balance <= TARGET_ACCOUNT_BALANCE) {
	  console.log("##########################################################################")
	  console.log("#                                                                        #")
	  console.log("#  while your new account will be credited with test tokens (takes 5min) #")
	  console.log("#  please have a look at https://forum.cardano.org/c/developers          #")
	  console.log("#  to learn new things and pass on your knowledge to others.             #")
	  console.log("#                                                                        #")
	  console.log("##########################################################################")
  }
  ...
  if (CONTRACTADDRESS == undefined) {
    ...
    console.log("New contract successfully compiled and deployed")
    ...
    nconf.save(function (err) {
  	if (err) {
  	  ...
  	  return;
  		...
  		console.log("was stored in myData.json and will be used for future operations")
    });
  ...
  console.log("Now going to interact with contract " + CONTRACTADDRESS)  
